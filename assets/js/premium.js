...
const v=document.querySelector('[data-hero-video]'),s=document.querySelector('[data-sound-toggle]');s?.addEventListener('click',()=>{if(!(v instanceof HTMLVideoElement))return;v.closest('.reference-hero__media')?.classList.add('is-playing');v.muted=!v.muted;s.classList.toggle('is-on',!v.muted);s.setAttribute('aria-label',v.muted?'Ton einschalten':'Ton ausschalten');v.play().catch(()=>{})});
})();
